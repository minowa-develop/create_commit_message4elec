...
export async function registFavorite() {
    // read history
    var favoriteList = await readList();
...
async function readList() {
    let rawList = await window.myAPI.readFile(FAVORITE_FILE);
    let list = [];
...
export async function showFavoriteList() {
    // read history
    let favoriteList = await readList();
    let table = ElementGetter.getTableElementById('favorite_area');
    // reset tr
    ...
    for (var i = favoriteList.length - 1; 0 <= i; i--) {
        ...
        // title
        let tdFilename = ElementSousa.createTdCallSetForm(TitleMaker.make(favoriteList[i]), favoriteList[i]);
 ...
 * @returns
 */
function createTdCallDelFavorite(msg, favoriteList, delindex) {
    let button = ElementSousa.createButtonElement(msg);
