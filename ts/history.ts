...
export async function registHistory(){
  // read history
  let historyList: Array<HistoryData> = await readList();
 ...
 * @returns 
 */
async function readList(): Promise<HistoryData[]>{
  let rawList = await window.myAPI.readFile(HISTORY_FILE) as Array<HistoryData>;
